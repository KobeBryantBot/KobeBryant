...
> 事实上，例如常用的Python里面，`async`和`await`的异步是协程实现的，依旧是单线程，只能在I/O密集型环境下提高性能，而除了网络数据包请求等待这种I/O密集型任务，还会有大量时间花在解析数据包内容这种CPU密集型环境上。  
> 然而，在Python里面，CPU密集型环境使用异步不仅不会带来性能的提升，还会带来性能的下降，因为处理依旧是单线程，而创建异步需要额外开销。
