...
public:
    ...
    : ThreadPoolBase(threads, Init{}, Invoke{}) {}

    ThreadPoolBase(size_t threads, Init init)
        requires(std::is_default_constructible_v<Invoke>)
    : ThreadPoolBase(threads, std::move(init), Invoke{}) {}

    ThreadPoolBase(size_t threads, Init init, Invoke invoke) : mIniter(std::move(init)), mInvoker(std::move(invoke)) {
